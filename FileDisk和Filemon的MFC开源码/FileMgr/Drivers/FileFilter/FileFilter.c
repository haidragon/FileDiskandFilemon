...
// file is on a drive that FileFilter is monitoring FileFilter would be
...
//
// These are prototypes for FileFilter's Fast I/O hooks. The originals 
...
//
// FileFilter keeps track of the number of distinct output buffers that
...
//
// This FileFilter's Fast I/O dispatch table. Note that NT assumes that
    ...
    )
{
    ...
    if( MaxLog == NumLog ) {
        ...
        DbgPrint(("FileFilter ***** Dropping records at sequence number %d\n", Sequence ));
        CurrentLog->Len = 0;
        return; 
    }
    ...
    )
{
    ...
    if( !NT_SUCCESS( ntStatus ) ) {
        ...
        DbgPrint(("FileFilter: Could not open %s\n", FsType == NPFS ? "NPFS" : "MSFS", ntStatus ));

        return FALSE;
    }

    DbgPrint(("FileFilter:  opened the root directory!!! handle: %x\n", ntFileHandle));   
    ...
    if( !NT_SUCCESS( ntStatus )) {
        ...
        DbgPrint(("FileFilter: Could not get fileobject from %s handle: %x\n", 
                  FsType == NPFS ? "NPFS" : "MSFS", ntStatus ));
    ...
    if( ! fileSysDevice ) {
        ...
        DbgPrint(("FileFilter: Could not get related device object for %s: %x\n", 
                  FsType == NPFS ? "NPFS" : "MSFS", ntStatus ));
    ...
    if( !NT_SUCCESS(ntStatus) ) {
        ...
        DbgPrint(("FileFilter: failed to create associated device %s: %x\n", 
                  FsType == NPFS ? "NPFS" : "MSFS", ntStatus ));
    ...
    if( !topAttachDevice )  {
        ...
        //
        DbgPrint(("FileFilter: Connect with Filesystem failed: %s (%x) =>%x\n", 
        ...
        DbgPrint(("FileFilter: Successfully connected to Filesystem device %s\n",
    ...
    )
{
    ...
    if( DriveHookDevices[Drive] == NULL )  {
        ...
        if( !NT_SUCCESS( ntStatus ) ) {
            ...
            DbgPrint(("FileFilter: Could not open drive %c: %x\n", 'A'+Drive, ntStatus ));
            return FALSE;
        }
        ...
        if( !NT_SUCCESS( ntStatus )) {
            ...
            DbgPrint(("FileFilter: Could not get fileobject from handle: %c\n", 'A'+Drive ));
            ZwClose( ntFileHandle );
            return FALSE;
        }
        ...
        if( ! fileSysDevice ) {
            ...
            DbgPrint(("FileFilter: Could not get related device object: %c\n", 'A'+Drive ));
            ObDereferenceObject( fileObject );
            ZwClose( ntFileHandle );
            return FALSE;
        }
        ...
        if( !NT_SUCCESS(ntStatus) ) {
            ...
            DbgPrint(("FileFilter: failed to create associated device: %c\n", 'A'+Drive ));   
        ...
        if( !NT_SUCCESS(ntStatus) )  {
            ...
            // Couldn' attach for some reason
            //
            DbgPrint(("FileFilter: Connect with Filesystem failed: %c (%x) =>%x\n", 
            ...
            // already
            // 
            DbgPrint(("FileFilter: Successfully connected to Filesystem device %c\n", 'A'+Drive ));
        }
    ...
    ) 
{
    ...
    if( hookExt->Type == GUIINTERFACE ) {
        ...
        case IOCTL_FILEMON_SETDRIVES:
            ...
            //
            DbgPrint (("FileFilter: set drives\n"));
        ...
        case IOCTL_FILEMON_STOPFILTER:
            ...
            // Turn off logging
            //
            DbgPrint(("FileFilter: stop logging\n"));
            FilterOn = FALSE;
            break;
        ...
        case IOCTL_FILEMON_STARTFILTER:
            ...
            // Turn on logging 
            //
            DbgPrint(("FileFilter: start logging\n"));
            FilterOn = TRUE;
            break;
        ...
        case IOCTL_FILEMON_SETFILTER:
            ...
            //
            DbgPrint(("FileFilter: set filter\n"));
        ...
        case IOCTL_FILEMON_ZEROSTATS:
            ...
            //
            DbgPrint (("FileFilter: zero stats\n"));
        ...
        case IOCTL_FILEMON_GETSTATS:
			...
			{
				DbgPrint (("FileFilter: There is no unread data\n"));
        ...
        default:
            ...
            // Unknown control
            // 
            DbgPrint (("FileFilter: unknown IRP_MJ_DEVICE_CONTROL\n"));
    ...
    ) 
{
    ...
    for( i = 0; i < 26; i++ ) {
        ...
        if( SourceDevice == DriveHookDevices[i] ) {
            ...
            DbgPrint(("FileFilter: Detaching from drive: %c\n", 
    ...
    )
{
    ...
    case IRP_MJ_CREATE:
        ...
        DbgPrint(("FileFilter: IRP_MJ_CREATE\n"));
    ...
    case IRP_MJ_CLOSE:
        ...
        DbgPrint(("FileFilter: IRP_MJ_CLOSE\n"));
    ...
    case IRP_MJ_DEVICE_CONTROL:
        ...
        //
        DbgPrint (("FileFilter: IRP_MJ_DEVICE_CONTROL\n"));
...
{
    WCHAR                  deviceLinkBuffer[]  = L"\\DosDevices\\FileFilter";
    ...
    DbgPrint(("FileFilter.SYS: unloading\n"));
    ...
    DbgPrint(("FileFilter.SYS: deleted devices\n"));
    ...
    DbgPrint(("FileFilter.SYS: freed memory\n"));
}
    ...
    )
{
    ...
    WCHAR                   deviceNameBuffer[]  = L"\\Device\\FileFilter";
    UNICODE_STRING          deviceNameUnicodeString;
    WCHAR                   deviceLinkBuffer[]  = L"\\DosDevices\\FileFilter";
    UNICODE_STRING          deviceLinkUnicodeString;
    ULONG                   i;

    DbgPrint (("FileFilter.SYS: entering DriverEntry\n"));
    ...
    if(NT_SUCCESS(ntStatus)) {
        ...
        if(!NT_SUCCESS(ntStatus)) {
            ...
            DbgPrint (("FileFilter.SYS: IoCreateSymbolicLink failed\n"));
...
#if DBG        
        //
        // Driver unload is only set if we are debugging FileFilter. This is 
        ...
        //
        DbgPrint(("FileFilter: Failed to create our device!\n"));
        return ntStatus;
    }
